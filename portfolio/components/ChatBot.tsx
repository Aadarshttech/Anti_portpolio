            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="mb-4 w-[340px] sm:w-[400px] bg-[#0A0A0A]/90 backdrop-blur-xl border border-white/10 rounded-3xl shadow-[0_0_40px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col rings-1 ring-white/5"
                    >
                        {/* Header */}
                        <div className={`p-4 flex items-center justify-between border-b border-white/5 ${isWorksPage ? 'bg-white/5' : 'bg-gradient-to-r from-indigo-500/10 to-purple-500/10'}`}>
                        ...
                        {/* Messages Area */}
                        <div className="flex-grow h-[360px] overflow-y-auto p-5 space-y-5 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                            {messages.map((m, i) => (
                                <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                                    <div className={`flex gap-3 max-w-[85%] ${m.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                                        ...
                                        <div className={`p-3.5 rounded-2xl text-[14px] leading-relaxed whitespace-pre-wrap shadow-sm ${m.role === "user" ? "bg-white text-black rounded-tr-sm" : "bg-white/5 text-gray-200 border border-white/5 rounded-tl-sm backdrop-blur-md"}`}>
                                            {m.content}
                                        </div>
                                    </div>
                                </div>
                            ...
                            {isLoading && (
                                <div className="flex justify-start">
                                    <div className="flex gap-3 max-w-[85%]">
                                        ...
                                        <div className="bg-white/5 text-gray-400 p-3.5 rounded-2xl rounded-tl-sm border border-white/5 flex items-center gap-2 backdrop-blur-md">
                        ...
                        <form onSubmit={handleSend} className="p-4 border-t border-white/5 bg-black/20">
                            <div className="relative flex items-center">
                                <input
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-full py-3.5 pl-5 pr-14 text-[14px] text-white placeholder-gray-500 focus:outline-none focus:border-white/20 focus:bg-white/10 transition-all shadow-inner"
                                />
                                <button
                                    type="submit"
                                    disabled={!input.trim() || isLoading}
                                    className={`absolute right-2 p-2.5 rounded-full transition-all duration-300 ${!input.trim() || isLoading ? 'text-gray-600 scale-95' : (isWorksPage ? 'bg-white text-black hover:bg-gray-200 hover:scale-105' : 'bg-indigo-500 text-white hover:bg-indigo-400 hover:scale-105')}`}
                                >
