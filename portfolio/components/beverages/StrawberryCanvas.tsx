...
interface StrawberryCanvasProps {
...
function getFrameConfig() {
    if (typeof window === "undefined") return { count: TOTAL_FRAMES, step: 1, dir: "strawberry-frames" };
    const isMobile = window.innerWidth < 768;
    // Mobile: load all frames but from the mobile-optimized (640px) directory
    // Desktop: load all frames from the full-res directory
    return {
        count: TOTAL_FRAMES,
        step: 1,
        dir: isMobile ? "strawberry-frames-mobile" : "strawberry-frames",
    };
}

export const StrawberryCanvas = ({
    scrollYProgress,
    className = "",
    onLoadComplete,
    onLoadProgress,
}: StrawberryCanvasProps) => {
